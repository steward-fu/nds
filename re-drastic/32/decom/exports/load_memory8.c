...
u8 load_memory8(memory_interface_struct *memory_interface,u32 address)

{
  ...
  if ((memory_interface->page_table[address >> 0xb] & 0x3fffffff) != 0) {
    return *(u8 *)(address + memory_interface->page_table[address >> 0xb] * 4);
  }
  ...
  if (uVar1 == '\x01') {
    ...
    uVar6 = address & pmVar5[uVar4].mask;
    ...
    *(uint *)((int)memory_interface->page_table + ((address & 0xfffff800) >> 9)) = uVar3;
    ...
    if (uVar1 == '\x02') {
                    /* WARNING: Could not recover jumptable at 0x080138d4. Too many branches */
