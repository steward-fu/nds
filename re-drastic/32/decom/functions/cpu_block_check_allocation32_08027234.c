...
u32 cpu_block_check_allocation32(cpu_struct *cpu,u32 address)

{
  ...
  if (0x1ffffff < address) {
    ...
    if ((puVar3 == (u32 *)0x0) || ((*puVar3 >> ((address << 0x1a) >> 0x1b) & 3) == 0)) {
      uVar2 = 0;
    }
  ...
  if ((((cpu->memory_interface).memory)->itcm_code_bitmap_fine[(address << 0x11) >> 0x17] >>
       ((address << 0x1a) >> 0x1b) & 3) == 0) {
    return 0;
  }
  uVar2 = cpu_block_modify_itcm(cpu,address);
  return uVar2;
}
