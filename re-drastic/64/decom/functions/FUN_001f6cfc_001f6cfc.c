...
int FUN_001f6cfc(long param_1,long param_2,int param_3)

{
  ...
  *(short *)(param_1 + 0xc6) = sVar4;
  iStack_c = luaD_rawrunprotected(param_1,resume,*(long *)(param_1 + 0x10) + (long)param_3 * -0x10);
  if (iStack_c == -1) {
    iVar12 = 2;
  }
  else {
    ...
    if (1 < iStack_c) {
      do {
        ...
        *(undefined8 *)(param_1 + 0xb0) = *(undefined8 *)(lVar11 + 0x28);
        iStack_c = luaD_rawrunprotected_constprop_0(param_1,&iStack_c);
        iVar12 = iStack_c;
      } while (1 < iStack_c);
    }
  }
