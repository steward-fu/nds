...
void FUN_001705bc(long param_1)

{
  ...
  if (cVar1 == '\x02') {
    uStack_c = *(uint *)(*(long *)(*(long *)(param_1 + 0x920) + 8) +
                        (ulong)*(uint *)(param_1 + 0x95c));
    uVar2 = (ulong)uStack_c;
    *(uint *)(param_1 + 0x95c) = *(uint *)(param_1 + 0x95c) + 4;
    ...
    sVar3 = read(*(int *)(param_1 + 0x93c),&uStack_c,4);
    if (sVar3 < 0) {
...
LAB_001705ec:
  if (lStack_8 - ___stack_chk_guard != 0) {
    ...
    __stack_chk_fail(uVar2,&__stack_chk_guard,lStack_8 - ___stack_chk_guard,0);
  }
  return;
}
