...
static int set_uart(int fd, int speed, int flow_ctrl, int databits, int stopbits, int parity)
{
    ...
    if (fd < 0) {
        error(SDL"invalid handle\n");
        return -1;
    }

#if !defined(UT)
    if (tcgetattr(fd, &options) != 0) {
        error(SDL"failed to get uart attributes\n");
        return -1;
    }
    ...
    case 2:
        ...
        break;
    default:
        error(SDL"invalid flow bit(0x%x)\n", flow_ctrl);
        return -1;
    }
    ...
    case 8:
        ...
        break;
    default:
        error(SDL"invalid data bit(0x%x)\n", databits);
        return -1;
    }
    ...
    case 'S':
        ...
        break;
    default:
        error(SDL"invalid parity bit(0x%x)\n", parity);
        return -1;
    }
    ...
    case 2:
        ...
        break;
    default:
        error(SDL"invalid stop bit(0x%x)\n", stopbits);
        return -1;
    }
...
static int init_uart(int fd, int speed, int flow_ctrl, int databits, int stopbits, int parity)
{
    ...
    if (set_uart(fd, speed, flow_ctrl, databits, stopbits, parity) < 0) {
        error(SDL"failed to set joystick device\n");
        return -1;
    }
...
static int read_uart(int fd, char *buf, int len)
{
    ...
    if ((fd < 0) || !buf || (len <= 0)) {
        error(SDL"invalid parameters(0x%x, 0x%x, 0x%x)\n", fd, buf, len);
        return -1;
    }
...
static int parse_serial_buf(const char *cmd, int len)
{
    ...
    if (!cmd || (len < FRAME_LEN)) {
        error(SDL"invalid parameters(0x%x, 0x%x)\n", cmd, len);
        return - 1;
    }
...
int JoystickInit(void)
{
    ...
    if (myjoy.thread == NULL) {
        error(SDL"failed to create thread for joystick\n");
        return -1;
    }
#endif
